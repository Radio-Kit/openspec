...
Firmware and app SHALL eliminate the fixed startup delay after connection so the device config becomes available as soon as the transport is ready: the device pushes CONF_DATA/VAR_DATA on BLE client subscribe, and the app consumes pushed config immediately while keeping a fast request fallback.
